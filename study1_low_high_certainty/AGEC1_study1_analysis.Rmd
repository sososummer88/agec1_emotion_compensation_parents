...
```{r packages}
library (ggplot2) ; library ('car'); library ('lme4');library ("mediation"); library ("foreign") ; library ("sciplot"); library ('lmerTest') ; library ("tidyr"); library ("plyr");library ("dplyr")
```
...
d = read.csv("Parenting_sudy_8_trial_one_scenario_3.10.15.csv",skip =0,na.strings=c("NA","NaN", " "))

d$condition = factor(d$condition, label = c("low partner response", "high partner response", "Control"))

d$par = factor(d$par , label = c("Low Intensity", "High Intensity"))


d= subset (d, screen ==1)
d1 = subset (d , scenario <=2) 
d2 = subset (d, scenario >=3)
...
sd= d %>%
  group_by(par) %>%
  summarise(
    express = sd(express, na.rm = TRUE)) 
sd


#high intensity parenting scenarios 
r = lm (express~ condition , d1) ; summary (r)
confint (r, level = .95)
...
r = lm (exfeel~ condition , d1) ; summary (r)
confint (r, level = .95)
...
sd
# low intensity parenting scenarios 
r = lm (feel~ condition , d2) ; summary (r)
confint (r, level = .95)

r = lm (express~ condition , d2) ; summary (r)
confint (r, level = .95)

r = lm (exfeel~ condition , d2) ; summary (r)
confint (r, level = .95)
...
d1 = d[,c(20,22:24)] # clean the data frame to make the transformation easier. Later we would have to add the moderators into the first stage 
head(d1)
...
```{r moderating restuls for compansation}


str(d$par)

d1 = subset (d, par == "Low Intensity")
d1=subset (d1, condition != "Control")


d1$child_amount = as.integer(d1$child_amount)

r = lm (express ~ condition*age , d1); summary (r)
...
```{r expression regulation }


dfc <- summarySE(d, measurevar="exfeel", groupvars=c("condition","par"))
...
ggplot(dfc, aes(x=condition, y=exfeel, fill=condition)) + 
    geom_bar(position=position_dodge(), stat="identity", colour="black" ) +
    labs(title="Parenting Scenarios")+
